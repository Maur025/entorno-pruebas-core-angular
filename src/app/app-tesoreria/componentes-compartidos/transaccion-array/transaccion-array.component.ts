...
export class TransaccionArrayComponent {
  ...
  cambiaOperacion(indice, value) {
    ...
    this.medioTransferenciaService?.habilitados()?.subscribe(
      (response: ApiResponseStandard) => {
        ...
        const id = "thBanco_" + indice;
        const thBanco = document.getElementById(id);
        if (this.destino == "CAJA") {
          if (thBanco) thBanco.style.display = "none";
          const transfer = this.transferMediumDataSelect.find(
            (tm) => tm["destino"] == "CAJA"
          );
          ...
          this.formData()
            .at(indice)
            .get("medioTransferenciaId")
            .setValue(transfer["id"]);
          this.getCajas();
  ...
  getCajas() {
    this.cajaService.habilitados().subscribe(
      (response: ApiResponseStandard) => {
        this.cajaList =
          this.responseHandlerService?.handleResponseAsArray(response);
        this.cajaList = this.cajaList.filter(
          (cl) => cl["id"] !== this.bancoCajaId
        );
      },
      (error: ErrorResponseStandard) =>
        this.notificacionService.alertError(error)
  ...
  getBancosList() {
    this.bancoService.habilitados().subscribe(
      (response: ApiResponseStandard) => {
        this.bancoList =
          this.responseHandlerService?.handleResponseAsArray(response);
        this.bancoList = this.bancoList.filter(
          (bl) => bl["id"] !== this.bancoCajaId
        );
      },
      (error: ErrorResponseStandard) =>
        this.notificacionService.alertError(error)
  ...
  getCuentaBancoList(idBanco) {
    this.cuentaBancoService
      .getCuentasBanco(1000, 0, "nroCuenta", false, "", idBanco)
      .subscribe(
        (response: ApiResponseStandard) => {
          this.cuentaBancoList = response?.data || [];
          this.isReadyAccount = false;
        },
        (error: ErrorResponseStandard) => {
          this.notificacionService?.alertError(error);
          this.isReadyAccount = false;
  ...
  selectCajaBancoList(e) {
    if (e != undefined) {
      this.getCuentaBancoList(e);
      this.isReadyAccount = true;
    } else {
      this.isReadyAccount = false;
    }
  }

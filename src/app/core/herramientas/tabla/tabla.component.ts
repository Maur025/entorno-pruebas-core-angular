...
export class TablaComponent implements OnInit {
	...
	classTable: string =
		'table mb-0 table-hover align-middle nowrap data-table table-condensed'
	...
	buscarKeyDown(buscar) {
		if (!this.inputBuscar || this.inputBuscar == '' || buscar) {
			this.buscar = true
			if (this.filtros)
				this.inputBuscar || this.inputBuscar != ''
					? (this.filtros.keyword = this.inputBuscar)
	...
	exportar(tipo) {
		this.datosService[this.exportReport](tipo, this.filtros).subscribe(
			response => {
				if (tipo == 'XLSX')
	...
	exportarPlantilla() {
		this.datosService.exportarPlantilla().subscribe(
			response => {
				this.archivosService.generar64aExcel(
					response['content'].content,
					response['content'].name

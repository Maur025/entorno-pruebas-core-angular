...
export class AperturaCierreService {
  ...
  filterRecords = (
		...
		return this.apiService.tesoreria?.post(`${this.prefix}/${this.apiName}/filter?${queryParams}`,filterData);
	}

	toggleStatus = (data: object): Observable<ApiResponseStandard> => {
		return this.apiService.tesoreria?.post(`${this.prefix}/${this.apiName}/habilita`,data);
	}

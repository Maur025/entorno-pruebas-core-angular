...
export class OperacionesComponent implements OnInit {
	...
	ngOnChanges() {
		if (this.montoPagar && this.operaciones.controls?.length > 0)
			this.calcularMontos()
	}
	...
	getCajas() {
		this.cajaService.aperturadas().subscribe(
			data => {
				this.listaCajas = data.content
				if (this.cajaIdNoUse) {
					this.listaCajas.splice(
						this.listaCajas.findIndex(obj => obj.id === this.cajaIdNoUse),
						1
					)
				}
			},
	...
	getMedioTransferencias() {
		this.medioTransferenciaService.habilitados().subscribe(
			data => {
				this.listaMedioTransferencias = data.content.filter(
					item => item.medio !== 'EFECTIVO'
				)
			},
			error => this.notificacionService.alertError(error)
		)
	...
	updateRadioButton = (ind: number): void => {
		const selectedValue = this.operaciones?.value[ind]
		this.radioButtons?.forEach((element: ElementRef, index: number) => {
			const radioButton = element.nativeElement as HTMLInputElement
			if (this.tipoOperacion[index]?.value === selectedValue?.tipoOperacion) {
				this.renderer.setAttribute(radioButton, 'checked', 'true')
			} else {
				this.renderer.removeAttribute(radioButton, 'checked')
			}
		})
	}
	...
	cambiaOperacion(index) {
		...
		if (
			this.operaciones.controls[index]['controls']['tipoOperacion'].value !=
			null
		) {
			if (
				this.operaciones.controls[index]['controls']['tipoOperacion'].value ==
				...
				this.operaciones
					.at(index)
					['addControl']('bancoId', new FormControl(null, Validators.required))
				this.operaciones
					.at(index)
					['addControl'](
						'cuentaBancoId',
				...
				this.operaciones
					.at(index)
					['addControl']('cajaId', new FormControl(null, Validators.required))
	...
	calcularMontos() {
		...
		if (this.montoTotal != this.montoPagar) {
			const indexUltimaOperacion = this.operaciones.controls.length - 1
			this.operaciones.controls[indexUltimaOperacion]['controls'][
				'monto'
			].setValue(
				this.operaciones.controls[indexUltimaOperacion]['controls']['monto']
					.value +
					(this.montoPagar - this.montoTotal)
			)
			this.calcularMontos()
		}
	}
	...
	cambioCaja(index) {
		if (this.operaciones.controls[index]['controls']['cajaId'].value != null) {
			this.operaciones.controls[index]['controls']['saldo'].setValue(
				this.listaCajas.find(
					fo =>
						fo.id ===
						this.operaciones.controls[index]['controls']['cajaId'].value
				).saldo
			)
			this.listaOperaciones[index].datos = this.listaCajas.find(
				fo =>
					fo.id === this.operaciones.controls[index]['controls']['cajaId'].value
			)
	...
	cambiaBanco(index) {
		...
		if (this.operaciones.controls[index]['controls']['bancoId'].value != null) {
			this.operaciones.controls[index]['controls']['cuentaBancoId'].setValue(
				null
			)
			this.cuentaBancoService
				.getCuentasBanco(
					...
					false,
					'',
					this.operaciones.controls[index]['controls']['bancoId'].value
				)
				.subscribe(
					data => {
	...
	cambioCuentaBancaria(index) {
		if (
			this.operaciones.controls[index]['controls']['cuentaBancoId'].value !=
			null
		) {
			this.operaciones.controls[index]['controls']['saldo'].setValue(
				this.listaOperaciones[index].listaCuentasBanco.find(
					fo =>
						fo.id ===
						this.operaciones.controls[index]['controls']['cuentaBancoId'].value
				).saldo
			)
			this.listaOperaciones[index].datos = this.listaOperaciones[
				index
			].listaCuentasBanco.find(
				lop =>
					lop.id ===
					this.operaciones.controls[index]['controls']['cuentaBancoId'].value
			)

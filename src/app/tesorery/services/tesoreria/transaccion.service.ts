...
export class TransaccionService {
  ...
  getAll(
    ...
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}?size=${size}&page=${
        page - 1
      }&sortBy=${sortBy}&descending=${descending}&keyword=${keyword}&estadoSagaId=${estadoSagaId}`
    );
  }
  ...
  getTransaccionCredito(transaccionId: any) {
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}/${transaccionId}/credito`
    );
  }
  getTransaccionEstado(transaccionId: any) {
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}/${transaccionId}/estado`
    );
  }
  getTransaccionEstadoCancelado(transaccionId: any) {
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}/${transaccionId}/estado/cancelado`
    );
  }
}

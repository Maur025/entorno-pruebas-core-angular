...
export class AplicacionAnticipoService {
  apiName: string = "aplicacionAnticipo";
  entitys: string = "Aplicacion de Anticipos";
  ...
  filter(
    ...
    return this.apiService.tesoreria.post(
      `/${this.apiName}/filter?page=${page}&size=${size}&sortBy=${sortBy}&descending=${descending}`,
      filtros
    );
  }
  ...
  deleteUpdate(id: string | number) {
    console.log("AQUUUU----");
    return this.apiService.tesoreria.delete(
      `${this.apiUrl}${this.prefix}/${this.apiName}/${id}/actualizarSaldo`
    );
  }
}

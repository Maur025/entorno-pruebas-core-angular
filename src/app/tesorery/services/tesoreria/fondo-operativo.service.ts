...
export class FondoOperativoService {
  ...
  deshabilitar(datos: any, id: string | number): Observable<any> {
    ...
    return this.apiService.tesoreria.put(
      `${this.apiUrl}${this.prefix}/${this.apiName}/${datos.id}/habilita`,
      datos
    );
  }
  ...
  aperturar(id: string = "") {
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}/aperturar/${id}`
    );
  }

  cerrarFondo(fondoId: any) {
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}/${fondoId}/cierre`
    );
  }

  aperturados() {
    return this.apiService.tesoreria.get(
      `${this.apiUrl}${this.prefix}/${this.apiName}/listarApertutados`
    );
  }
}

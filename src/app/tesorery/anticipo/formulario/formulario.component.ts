...
export class FormularioComponent implements OnInit {
	...
	setOnChangeOperations = (value: any): void => {
	...
	getFondoApertudados() {
		this.fondoOperativoService.aperturados().subscribe(
			fondo => {
	...
	cambiaOperacion(index) {
		...
		this.calcularMontos()
		if (
			this.form['monto'].value != null &&
			this.form['monto'].value - this.montoTotal
		) {
			this.operaciones.controls[index]['controls']['monto'].setValue(
				Number(this.form['monto'].value - this.montoTotal)
			)
		} else {
			this.operaciones.controls[index]['controls']['monto'].setValue(0)
		}
	...
	cambioFondoOperativo(index) {
		if (
			this.operaciones.controls[index]['controls']['fondoOperativoId'].value !=
			null
		) {
			this.listaOperaciones[index].datos = this.listaFondoAperturados.find(
				fo =>
					fo.id ===
					this.operaciones.controls[index]['controls']['fondoOperativoId'].value
			)
	...
	guardar() {
		...
		if (this.formGroup.valid) {
			if (this.formGroup.get('monto').value > 0) {
				if (this.montoTotal === parseFloat(this.formGroup.get('monto').value)) {
					if (this.tipo == 'aplicacion') {
						...
						this.aplicacionAnticipoService.register(data).subscribe(
							(res: any) => {
						...
						this.tipo == 'nuevo' &&
						this.montoTotal == this.form['monto'].value
					) {
						let data = {
							...this.formGroup.value,
							monto: this.formGroup.get('monto').value || 0,
						...
						this.anticipoService.register(data).subscribe(
							(res: any) => {
								this.notificacionService.successStandar()
								this.alGuardar.emit(res)
							},

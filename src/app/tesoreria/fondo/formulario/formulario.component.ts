...
this.FondoresponsablesService.getAll(100, 1, 'fondoId', false, '').subscribe((res:any) => { this.fondo_responsables = res.content; });
this.FondocentrodecostosService.getAll(100, 1, 'fondoId', false, '').subscribe((res:any) => { this.fondo_centrodecostos = res.content; });
this.FondoformacobroService.getAll(100, 1, 'fondoId', false, '').subscribe((res:any) => { this.fondo_forma_cobro = res.content; });
  }

  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],nombre:["",[Validators.required] ],direccion:["",[Validators.required] ],descripcion:["",[Validators.required] ],fondoTipoId:["",[Validators.required] ],solicitudId:["",[Validators.required] ],fondoResponsables:["",[] ],fondoCentrodecostos:["",[] ],fondoFormaCobro:["",[] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,direccion:this.dataEdit.direccion,descripcion:this.dataEdit.descripcion,fondoTipoId:this.dataEdit.fondoTipoId,solicitudId:this.dataEdit.solicitudId,fondoResponsables:this.dataEdit.fondoResponsables,fondoCentrodecostos:this.dataEdit.fondoCentrodecostos,fondoFormaCobro:this.dataEdit.fondoFormaCobro});

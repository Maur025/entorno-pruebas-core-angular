...
export class FormularioComponent implements OnInit {
  ...
  cargarArrays()
  {
    this.DocumentotipoService.getAll(100, 1, 'nombre', false, '').subscribe((res:any) => { this.documento_tipo = res.content; });
  }

  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],documentoTipoId:["",[Validators.required] ],referenciaId:["",[Validators.required,Validators.minLength(0),Validators.maxLength(255)] ],observaciones:["",[Validators.maxLength(255)] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,documentoTipoId:this.dataEdit.documentoTipoId,referenciaId:this.dataEdit.referenciaId,observaciones:this.dataEdit.observaciones});
      this.rel_prefix = "/documento/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.DocumentoService.find(id).subscribe((result:any) => {
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.DocumentoService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.DocumentoService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

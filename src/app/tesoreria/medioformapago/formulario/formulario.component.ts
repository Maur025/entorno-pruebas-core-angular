...
@Component({
  selector: "app-formulario-medioformapago",
...
export class FormularioComponent implements OnInit {
  ...
  constructor(
    ...
    private MedioformapagoService: MedioformapagoService,
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],nombre:["",[Validators.required,Validators.minLength(2),Validators.maxLength(255)] ],descripcion:["",[Validators.maxLength(255)] ],formaPagoId:["",[] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,descripcion:this.dataEdit.descripcion,formaPagoId:this.dataEdit.formaPagoId});
      this.rel_prefix = "/medio_forma_pago/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.MedioformapagoService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,descripcion:this.dataEdit.descripcion,formaPagoId:this.dataEdit.formaPagoId});
          this.rel_prefix = "/medio_forma_pago/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.MedioformapagoService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.MedioformapagoService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

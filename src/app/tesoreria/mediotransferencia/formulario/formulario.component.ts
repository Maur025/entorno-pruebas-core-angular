...
@Component({
  selector: "app-formulario-mediotransferencia",
...
export class FormularioComponent implements OnInit {
  ...
  ngOnInit(): void {    
    ...
    if (this.dataEdit != null) {
      ...
      this.rel_prefix = "/medio_transferencia/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.MediotransferenciaService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,descripcion:this.dataEdit.descripcion});
          this.rel_prefix = "/medio_transferencia/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.MediotransferenciaService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.MediotransferenciaService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

...
@Component({
  selector: "app-formulario-formapago",
...
export class FormularioComponent implements OnInit {
  ...
  ngOnInit(): void {    
    ...
    if (this.dataEdit != null) {
      ...
      this.rel_prefix = "/forma_pago/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.FormapagoService.find(id).subscribe((result:any) => {
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.FormapagoService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.FormapagoService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

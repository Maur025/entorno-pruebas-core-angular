...
import { DeudorcuentaService } from "../servicios/deudorcuenta.service";
import { DeudorService } from '../servicios/deudor.service';
import { CuentaService } from '../servicios/cuenta.service';
@Component({
  selector: "app-formulario-deudorcuenta",
...
cuenta:any = [];
  ...
  constructor(
    ...
    private DeudorcuentaService: DeudorcuentaService,
    private DeudorService: DeudorService,private CuentaService: CuentaService
  ) {}
...
this.CuentaService.getAll(100, 1, 'id', false, '').subscribe((res:any) => { this.cuenta = res.content; });
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],deudorId:["",[Validators.required] ],cuentaId:["",[Validators.required] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,deudorId:this.dataEdit.deudorId,cuentaId:this.dataEdit.cuentaId});
      this.rel_prefix = "/deudor_cuenta/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.DeudorcuentaService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,deudorId:this.dataEdit.deudorId,cuentaId:this.dataEdit.cuentaId});
          this.rel_prefix = "/deudor_cuenta/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.DeudorcuentaService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.DeudorcuentaService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

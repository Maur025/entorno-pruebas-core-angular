...
@Component({
  selector: "app-formulario-tiposolicitud",
...
export class FormularioComponent implements OnInit {
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],nombre:["",[Validators.required,Validators.minLength(2),Validators.maxLength(255)] ],descripcion:["",[Validators.maxLength(255)] ],montofijo:[false,[Validators.required] ],monedaId:["",[Validators.required] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,descripcion:this.dataEdit.descripcion,montofijo:this.dataEdit.montofijo,monedaId:this.dataEdit.monedaId});
      this.rel_prefix = "/tipo_solicitud/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.TiposolicitudService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,descripcion:this.dataEdit.descripcion,montofijo:this.dataEdit.montofijo,monedaId:this.dataEdit.monedaId});
          this.rel_prefix = "/tipo_solicitud/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.TiposolicitudService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.TiposolicitudService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

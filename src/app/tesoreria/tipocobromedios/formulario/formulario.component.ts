...
@Component({
  selector: "app-formulario-tipocobromedios",
...
medio_transferencia:any = [];
  ...
  constructor(
    ...
    private TipocobromediosService: TipocobromediosService,
    private TipocobroService: TipocobroService,private MediotransferenciaService: MediotransferenciaService
  ) {}
...
this.MediotransferenciaService.getAll(100, 1, 'nombre', false, '').subscribe((res:any) => { this.medio_transferencia = res.content; });
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],tipoCobroId:["",[Validators.required] ],medioTransferenciaId:["",[Validators.required] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,tipoCobroId:this.dataEdit.tipoCobroId,medioTransferenciaId:this.dataEdit.medioTransferenciaId});
      this.rel_prefix = "/tipo_cobro_medios/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.TipocobromediosService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,tipoCobroId:this.dataEdit.tipoCobroId,medioTransferenciaId:this.dataEdit.medioTransferenciaId});
          this.rel_prefix = "/tipo_cobro_medios/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.TipocobromediosService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.TipocobromediosService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

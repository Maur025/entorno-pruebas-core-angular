...
@Component({
  selector: "app-formulario-tipoidentificacion",
...
export class FormularioComponent implements OnInit {
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],sigla:["",[Validators.required,Validators.minLength(2),Validators.maxLength(255)] ],nombre:["",[Validators.required,Validators.minLength(2),Validators.maxLength(255)] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,sigla:this.dataEdit.sigla,nombre:this.dataEdit.nombre});
      this.rel_prefix = "/tipo_identificacion/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.TipoidentificacionService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,sigla:this.dataEdit.sigla,nombre:this.dataEdit.nombre});
          this.rel_prefix = "/tipo_identificacion/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.TipoidentificacionService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.TipoidentificacionService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

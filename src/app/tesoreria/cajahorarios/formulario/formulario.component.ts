...
this.UsuariosService.getAll(100, 1, 'nombre', false, '').subscribe((res:any) => { this.usuarios = res.content; });
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],cajaId:["",[Validators.required] ],horarioId:["",[Validators.required] ],responsableId:["",[Validators.required] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,cajaId:this.dataEdit.cajaId,horarioId:this.dataEdit.horarioId,responsableId:this.dataEdit.responsableId});
      this.rel_prefix = "/caja_horarios/"+this.dataEdit.id;
    }
    ...
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.CajahorariosService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,cajaId:this.dataEdit.cajaId,horarioId:this.dataEdit.horarioId,responsableId:this.dataEdit.responsableId});
          this.rel_prefix = "/caja_horarios/"+id;
          this.rel_id = id;
      });
    }
  }

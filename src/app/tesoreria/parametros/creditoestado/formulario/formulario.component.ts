...
estado_credito:any = [];
  ...
  ngOnInit(): void {
    this.CreditoService.getAll(100, 1, 'capital', false, '').subscribe((res:any) => { this.credito = res.content; });
this.EstadocreditoService.getAll(100, 1, 'nombre', false, '').subscribe((res:any) => { this.estado_credito = res.content; });
    this.formGroup = this.FormBuilder.group({id:["",[] ],fecha:["",[] ],creditoId:["",[] ],estadoCreditoId:["",[] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,fecha:this.dataEdit.fecha,creditoId:this.dataEdit.creditoId,estadoCreditoId:this.dataEdit.estadoCreditoId});
      this.rel_prefix = "/credito_estado/"+this.dataEdit.id;
    }
    ...
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.CreditoestadoService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,fecha:this.dataEdit.fecha,creditoId:this.dataEdit.creditoId,estadoCreditoId:this.dataEdit.estadoCreditoId});
          this.rel_prefix = "/credito_estado/"+id;
          this.rel_id = id;
      });
    }
  }

...
@Component({
  selector: "app-formulario-deudor",
...
export class FormularioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formGroup = this.FormBuilder.group({id:["",[] ],entidadId:["",[] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,entidadId:this.dataEdit.entidadId});
      this.rel_prefix = "/deudor/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.DeudorService.find(id).subscribe((result:any) => {
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.DeudorService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.DeudorService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

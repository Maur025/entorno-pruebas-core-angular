...
@Component({
  selector: "app-formulario-fondotipo",
...
export class FormularioComponent implements OnInit {
  ...
  constructor(
    ...
    private FondotipoService: FondotipoService,
  ...
  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],nombre:["",[Validators.required,Validators.minLength(2),Validators.maxLength(255)] ],descripcion:["",[Validators.minLength(2),Validators.maxLength(255)] ],esrotativo:[false,[Validators.required] ],esoperativo:[false,[Validators.required] ],conrendicion:[false,[Validators.required] ]});
    if (this.dataEdit != null) {
      ...
      this.rel_prefix = "/fondo_tipo/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.FondotipoService.find(id).subscribe((result:any) => {
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.FondotipoService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.FondotipoService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

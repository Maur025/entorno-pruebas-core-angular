...
import { AcreedorcuentaService } from "../servicios/acreedorcuenta.service";
import { AcreedorService } from '../servicios/acreedor.service';
import { CuentaService } from '../servicios/cuenta.service';
@Component({
  selector: "app-formulario-acreedorcuenta",
...
cuenta:any = [];
  ...
  constructor(
    ...
    private AcreedorcuentaService: AcreedorcuentaService,
    private AcreedorService: AcreedorService,private CuentaService: CuentaService
  ) {}
...
this.CuentaService.getAll(100, 1, 'id', false, '').subscribe((res:any) => { this.cuenta = res.content; });
  }

  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],acreedorId:["",[Validators.required] ],cuentaId:["",[] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,acreedorId:this.dataEdit.acreedorId,cuentaId:this.dataEdit.cuentaId});
      this.rel_prefix = "/acreedor_cuenta/"+this.dataEdit.id;
    }
    let id = this.route.snapshot.params['id'];
    if (this.rel_prefix && this.rel_field) this.formGroup.get(this.rel_field).disable();
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.AcreedorcuentaService.find(id).subscribe((result:any) => {
        ...
        if (Array.isArray(result.content))
          ...
          this.formGroup.setValue({id:this.dataEdit.id,acreedorId:this.dataEdit.acreedorId,cuentaId:this.dataEdit.cuentaId});
          this.rel_prefix = "/acreedor_cuenta/"+id;
          this.rel_id = id;
      });
    }
  }
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.AcreedorcuentaService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.AcreedorcuentaService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

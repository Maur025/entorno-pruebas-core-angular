...
export class FormularioComponent implements OnInit {
  ...
  cargarArrays()
  {
    this.HorariosdiaService.getAll(100, 1, 'horarioId', false, '').subscribe((res:any) => { this.horarios_dia = res.content; });
  }

  ngOnInit(): void {    
    this.cargarArrays();
    this.formGroup = this.FormBuilder.group({id:["",[] ],nombre:["",[Validators.required,Validators.minLength(2),Validators.maxLength(255)] ],horariosDia:["",[] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre,horariosDia:this.dataEdit.horariosDia});
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.HorariosService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.HorariosService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

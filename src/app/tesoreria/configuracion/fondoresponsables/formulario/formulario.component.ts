...
@Component({
  selector: "app-formulario-fondoresponsables",
...
fondo:any = [];
  ...
  constructor(
    ...
    private FondoresponsablesService: FondoresponsablesService,
    private UsuariosService: UsuariosService,private FondoService: FondoService
  ) {}
...
this.FondoService.getAll(100, 1, 'nombre', false, '').subscribe((res:any) => { this.fondo = res.content; });
    ...
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.FondoresponsablesService.find(id).subscribe((result:any) => {
        if (result.content.length == 0) return;
        this.dataEdit= result.content[0];
          this.formGroup.setValue({id:this.dataEdit.id,responsable_id:this.dataEdit.responsable_id,fondo_id:this.dataEdit.fondo_id,estado:this.dataEdit.estado});
          this.rel_prefix = "/fondoresponsables/"+id;
      });
    }
  }
  volver(){
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.FondoresponsablesService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.FondoresponsablesService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

...
caja_horarios:any = [];
  ...
  constructor(
    ...
    private FondoService: FondoService,private CajahorariosService: CajahorariosService
  ) {}
...
this.CajahorariosService.getAll(100, 1, 'caja_id', false, '').subscribe((res:any) => { this.caja_horarios = res.content; });
    ...
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.CajaService.find(id).subscribe((result:any) => {
        if (result.content.length == 0) return;
        this.dataEdit= result.content[0];
          this.formGroup.setValue({id:this.dataEdit.id,fondo_id:this.dataEdit.fondo_id,nombre:this.dataEdit.nombre,caja_horarios:this.dataEdit.caja_horarios});
          this.rel_prefix = "/caja/"+id;
      });
    }
  }
  volver(){
  ...
  guardar() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataEdit == null) {
        this.CajaService.register(sendData).subscribe(
          (res: any) => {
        ...
        this.CajaService.update(sendData, this.dataEdit.id).subscribe(
          (res: any) => {
            this.notificacionService.successStandar();

...
export class FormularioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formGroup = this.FormBuilder.group({id:["",[] ],nombre:["",[Validators.required,Validators.minLength(0),Validators.maxLength(100)] ]});
    if (this.dataEdit != null) {
      this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre});
      this.rel_prefix = "/estadocredito/"+this.dataEdit.id;
    }
    ...
    if (id != null && !this.esModal && id!="nuevo" ) {
      this.EstadocreditoService.find(id).subscribe((result:any) => {
        ...
        this.dataEdit= result.content[0];
          this.formGroup.setValue({id:this.dataEdit.id,nombre:this.dataEdit.nombre});
          this.rel_prefix = "/estadocredito/"+id;
      });
    }
  }
